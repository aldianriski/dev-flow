---
owner: Tech Lead (Aldian Rizki)
last_updated: 2026-04-26
update_trigger: Blueprint version bumped; new mode added
status: current
---

# Blueprint §10 — Workflow Modes

Index of mode reference files. Full detail in each sub-file.

| File | Sections |
|:-----|:---------|
| [10a-init.md](10a-init.md) | §16 — INIT Mode |
| [10b-harness-improvement.md](10b-harness-improvement.md) | §17 — Harness Continuous Improvement Protocol |
| [10c-resume.md](10c-resume.md) | §18 — Session Resume Protocol |
| [10d-migration-performance.md](10d-migration-performance.md) | §19 — Migration Safety · §20 — Performance Gate |
| [10e-hotfix.md](10e-hotfix.md) | §21 — Hardened Hotfix Mode |
| [10f-task-decomposer.md](10f-task-decomposer.md) | §22 — Task Decomposer · §23 — Sprint Mode |
