...
- **T5 close 2026-05-10** — Plugin propagation + lockstep MINOR bump 3.0.0 → 3.1.0. CONTEXT.md § Vocabulary adds `flow grill` row + extends `grill` row (strict vs batched variants); plugin.json + marketplace.json bumped lockstep per ADR-006. CHANGELOG v3.1.0 prepended (O5 flow + O8 reliability outcome lead per ADR-026; 7 components changed). README.md untouched (zero references to Sprint Promote / Flow Grill — no edit needed). Eval gates: cap-headroom 15 OK · 1 WARN (lean-doc 96/100) · 0 BREACH (improved from baseline 13/2/1; **0 NEW breach** per AC); eval-skills 13 pass / 3 fail (codemap-refresh + prime + release-patch missing ## Red Flags — all PRE-EXISTING since Sprint 037+ baseline; 0 NEW failures from T5). Manual MINOR sprint-less bump (4th instance: Sprint 052b T1 + Release 2.7.0 + Release 3.0.0 + this Release 3.1.0); release-patch NOT invoked per ADR-027 boundary. HITL gate fired pre-commit per pre-locked D-K; user `lock` 2026-05-10 approves.
...
- `.claude/CONTEXT.md` | T5 | EDIT — § Vocabulary adds `flow grill` row + extends `grill` row (strict vs batched variants per ADR-036 DEC-3) | risk: low | test: N/A
- `.claude-plugin/plugin.json` | T5 | EDIT — version 3.0.0 → 3.1.0 lockstep | risk: low | test: N/A
- `.claude-plugin/marketplace.json` | T5 | EDIT — version 3.0.0 → 3.1.0 lockstep | risk: low | test: N/A
- `docs/CHANGELOG.md` | T5 | EDIT — v3.1.0 block prepended (O5 + O8 outcome lead · 7 components · 4th-instance manual MINOR pattern note); frontmatter last_updated tag refreshed | risk: low (additive prepend) | test: N/A
