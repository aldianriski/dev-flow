...
- **T7 close 2026-05-10** — README "How You Use It" helicopter view section added (per user mid-sprint addition · D-A scope expansion locked at promote b1ae530). Section inserted between "How to Adopt" and "First Sprint Walkthrough"; initially ~83 lines (target ≤80). Contains: 30-second pitch · 3-layer mental model ASCII · top-level entry points table · end-to-end lifecycle ASCII · 4-bullet "good behavior" rules. Section reflects POST-Sprint-059 state (15 skills · 6 agents · `--grill` flag · Flow Grill loop). Banner/Roadmap/components-table edits intentionally NOT touched (T6 scope). README initial 178 → 261 lines.
- **T7-revised mid-T6 2026-05-10** — User context-cost feedback "readme.md need to trim for context optimal" surfaced post-T7 commit. Refactored: full helicopter content moved to NEW `docs/HOW-YOU-USE-IT.md` (100 lines · standalone canonical user-workflow doc). README "How You Use It" section trimmed 72 → 14 lines (kept brief 5-row entry-points table + pointer to full doc).
- **T7-second-trim mid-T6 2026-05-10** — User added "road map can remove now, all flow already done" + "other explenation redundant". Aggressive README trim: ## Roadmap section removed entirely (14 lines · v3.1.0+v4.0.0 multi-sprint plan obsoleted by completion); ## Skills (23 lines) + ## Agents (16 lines) + ## Hooks and Scripts (19 lines) = 58 lines combined → collapsed into single 9-line "## Skills · Agents · Hooks · Scripts" pointer section (canonical mappings live in USER-OUTCOMES.md + CONTEXT.md + ARCHITECTURE.md anyway); ## First Sprint Walkthrough 21 → 3 lines (1-line walkthrough flow + pointer to docs/HOW-YOU-USE-IT.md). **Bonus:** trim eliminated staleness — old Skills table had `architecture-grill` row (obsolete post-T1) and old Agents table had `dispatcher` row (obsolete post-T2); collapsed-pointer approach side-steps maintenance burden.
- **T7-third-trim mid-T6 2026-05-10** — User flagged "daily workflow and daily entry point is still redundant". ## Working on This Repo trimmed 12 → 2 lines: "Daily workflow pattern" 3-step block removed (duplicated the entry-points table in ## How You Use It · same 3 commands `/prime` · `/lean-doc-generator` · `/orchestrator`); section now contribution-context only (TODO.md ref · CONTRIBUTING + SUPPORT pointers · session-workflow blueprint pointer). Final README: 115 lines (peak 261 → final 115 · **-146 · -56% session-load cost reduction**). Pattern fully validated: single-source-of-truth + pointer-line fan-out across canonical docs (USER-OUTCOMES.md / CONTEXT.md / HOW-YOU-USE-IT.md / ARCHITECTURE.md) beats README-as-everything-encyclopedia.
...
- `README.md` | T7 (revised 3x mid-T6) | EDIT (178 → 261 → 115 lines · net **-63** vs pre-T7) — initial T7 +83 lines (helicopter inline); 1st trim moved helicopter to new doc (-58); 2nd trim removed Roadmap + collapsed Skills/Agents/Hooks-Scripts/Walkthrough into pointers (-79); 3rd trim removed Daily workflow pattern duplicate from Working-on-This-Repo (-9); final 115 lines · **-56%** session-load cost reduction vs peak. Stale arch-grill + dispatcher rows eliminated; daily-pattern duplicate eliminated | risk: low | test: N/A
- `docs/HOW-YOU-USE-IT.md` | T7 (mid-T6 trim) | NEW (100 lines) — canonical user-workflow doc; carries full helicopter view + "Where each component fits in your day" mapping table; pattern: README pointer + canonical-doc fan-out | risk: low (additive doc) | test: N/A
