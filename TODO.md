...
> Sprint 051a closed `460175b`. Next: Sprint 051b — TASK-122b templates + primer + TASK-126 untracked-files (T0 prereq, BLOCKING — docs/blueprint/ untracked → plugin install ships incomplete) + TASK-127 optimal usage workflow vision (fold-in).
>
> **Session-end audit (2026-05-08):** 5 slipped concerns surfaced + tracked: TASK-125 broader feature-usage audit (Sprint 053b) · TASK-126 untracked reconcile (Sprint 051b T0) · TASK-127 workflow vision (Sprint 051b fold-in) · TASK-128 token audit (Sprint 055b) · TASK-129 /prime behavior fix (Sprint 052 fold-in with F4) · release-debt resolution PROMOTED P2→P0 Sprint 052b.
...
- [ ] **TASK-126** — Untracked-files reconcile. Resolve pre-existing untracked files in repo: `AI_WORKFLOW_BLUEPRINT.md` · `AUDIT.md` · `AUDIT_PASS2.md` · `BASELINE_ASPECT.md` · `READINESS.md` · `STRATEGY_REVIEW.md` · `docs/blueprint/` (21 files — CRITICAL: bin/dev-flow-init.js copies blueprint/ to user-projects but it's not tracked → plugin install ships incomplete) · `docs/context/`. Per-file decision: commit / .gitignore / delete. **BLOCKS Sprint 051b** because 051b touches docs/blueprint/. Estimated S, layers `governance, docs`. → Sprint 051b T0 (fold-in as first sub-task) OR own micro-sprint before 051b.
...
- [ ] **TASK-129** — `/prime` skill behavior fix. Origin: user session 2026-05-08 — "prime skills need to fix first, no continue flow after load, is it correct? and after i use, prime always give to much summarize information, make use more token." Two fixes: (1) add `Next:` line to output format (active sprint → suggest `/orchestrator`; no sprint → suggest `/task-decomposer` or `/zoom-out`) — preserves read-only contract while restoring flow continuation hint. (2) add explicit anti-patterns against inline file summarization + re-reading unchanged files; reuse Sprint 045 lean-doc Step 0a SHA cache pattern via `.claude/.lean-doc-cache.json` for cache-hit semantics; trim sprint-plan read to frontmatter + Active-Sprint section only (~50 lines) instead of full file. Layers `skills, docs`. **Fold-in candidate Sprint 052** (alongside F4 orchestrator-wiring detection — same theme: skill flow continuation). → Sprint 052 fold-in OR own row if scope balloons.
...
Sprint 51b   →  Lean Architecture Templates + Primer (TASK-122b CLAUDE.md + ARCHITECTURE.md rewrites + blueprint/11-lean-architecture.md primer) · TASK-126 untracked-files reconcile (T0 prereq) · TASK-127 optimal usage workflow vision (fold-in)
Sprint 52    →  F4 wire orphan skills into orchestrator (tdd / refactor-advisor / diagnose / zoom-out / prime / release-manager phase detection) + F5 tech-debt rollover loop (TASK-123) + TASK-129 prime behavior fix (Next: line + anti-summarize anti-pattern + SHA cache)
