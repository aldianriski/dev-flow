...
test('Check 1 — errors when settings.local.json missing', () => {
  ...
  try {
    ...
    assert.ok(result.stdout.includes('BLOCK:'), 'should include BLOCK message');
    assert.ok(result.stdout.includes('settings.local.json'), 'should mention settings.local.json');
  } finally {
    teardown(dir);
  }
});
